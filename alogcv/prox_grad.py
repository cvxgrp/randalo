...
def _line_search(x, lambda_init, f, grad_f, prox_g):
    ...
    while True:
        ...
        zmx = z - x
        norm_z_m_x = torch.linalg.vector_norm(zmx)
        if f(z) <= f_x + grad_fx @ zmx + 1 / 2 / lambda_ * norm_z_m_x**2:
            err = torch.linalg.vector_norm(grad_fx + 1 / lambda_ * (zmx))
            break
...
def accel_prox_grad_w_linesearch(
    f: Callable[[torch.Tensor], torch.Tensor],
    grad_f: Callable[[torch.Tensor], torch.Tensor],
    prox_g: Callable[[torch.Tensor], torch.Tensor],
    x0: torch.Tensor,
...
def prox_grad(
    ...
    while err > epsilon and k < max_iters:
        ...
        k += 1
        grad_fx = grad_f(x)
        err = torch.linalg.vector_norm(grad_fx - 1 / lambda_ * (x - intermediate_x))
