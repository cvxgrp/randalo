import torch
import alogcv.admm as admm
import alogcv.prox_grad as prox_grad
...
class CVXPYSolver(Solver):
    def __init__(self, problem, y_parameter, variables, estimate):
        ...
        try:
            ...
            raise RuntimeError(
                "Could not import cvxpylayers. Please ensure it is"
...
def _compute_op_norm(A, x, iters):
    ...
    for _ in range(iters):
        ...
        if torch.abs(norm_x).item() <= 1e-6:
            raise RuntimeError(
                "Either uniform vector or provided "
